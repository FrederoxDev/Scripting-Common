...
export class AnimationPlayer {
    ...
    addAnimation(animation: Animation, onTick: AnimationCallback, onAction: AnimationActionCallback, playbackSpeed: number = 1) {
        this.playingAnimations.push(new AnimationInstance(animation, onTick, onAction, playbackSpeed));
    }
    ...
    private _tickAnimation(deltaTime: number, anim: AnimationInstance): boolean {
        anim.elapsed_time += deltaTime * anim.playback_speed;
    ...
    private _processFrameActions(anim: AnimationInstance, frameIdx: number) {
        ...
        /** Call any actions */
        currentFrame.actions.forEach(actionName => anim.onAction(actionName));
    }
